...
Um unseren selbst geschriebenen Code auszuführen, also zum Beispiel um die 2D-Grafiken auf dem Bildschirm anzuzeigen, verwenden wir einen *Browser* (Chrome, Firefox, Safari oder Edge).
...
. Plugin "p5.vscode" installieren: https://marketplace.visualstudio.com/items?itemName=samplavigne.p5-vscode
. Tutorial Source Code herunterladen von Download-URL: TODO. Die heruntergeladene ZIP-Datei entpacken.
. Entpackten Ordner in Visual Studio Code öffnen
. Die Datei index.html öffnen und in der Statuszeile auf "Go Live" klicken. Die index.html wird nun automatisch im Browser geöffnet.
