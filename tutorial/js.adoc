...
Der Computer wiederholt also den Schleifeninhalt solange, bis die Abbruchbedingung `false` ist. Ist diese `false`, wird die Schleife nicht mehr ausgeführt. Der Computer führt dann die nächste Anweisung nach der Schleife aus, bzw. beendet das Programm, wenn die Schleife am Ende steht.
...
a) Zeichne einen schwarzen quadratischen Hintergrund, welcher von weissen, vertikalen, 1-Pixel-breiten Linien in zehn Teile unterteilt wird.

b) Verändere das Programm so, dass du in einer Variable `parts` vorgeben kannst, in wieviele Teile die Fläche unterteilt wird.

c) Verändere das Programm so, dass jede zweite Linie 2px breit ist, wobei die erste Linie 2 Pixel breit ist.

Hinweis: Die Funktion `strokeWeight`

*Grid*

a) Zeichne ein quadratisches Gittermuster mit der Seitenlänge 10.

b) Fülle die x- und y-Achse mit Zahlen. Im Quadrat oben links steht dann eine 1. Rechts daneben eine 2, und so weiter; ganz rechts oben steht dann eine 10. Dasselbe Verhalten für die Spalte ganz links.
