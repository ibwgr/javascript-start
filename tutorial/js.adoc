...
Die Funktion `drawSmiley` wird innerhalbt der while Schleife insgesamt 10 mal aufgerufen. Bei jedem Aufruf wird für den x- und y-Parameter jeweils eine zufällige Zahl berechnet. Nachdem die while Schleife durch ist, wird zu guter Letzt wird ein Smiley in die Bildmitte gezeichnet.
...
Zeichne ein einfaches Haus mit mehreren Fenstern. Das Haus steht auf einer Blumenwiese mit grünem Gras. Die Blumen haben unterschiedliche Farben.

Falls dir Blumenwiesen nicht zusagen: Zeichne eine sandige Wüstenlandschaft mit Kakten unterschiedlicher Art (Form) und Grösse.
