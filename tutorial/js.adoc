...
Aus dem Mathematikunterricht kennst du etwas ähnliches, das kartesische Koordinatensystem (unten im Bild links). Im Unterschied zum kartesischen System, ist beim Koordinatensystem unserer Zeichenfläche der 0-Punkt ganz oben Links. Es gibt also nur ein Quadrant, und alle Punkte im Koordinatensystem haben positive x- und y-Werte.
...
Aufmerksame Leser werden bemerkt haben, dass sich im obigen Code ein neues Programmelement reingemogelt hat. Nämlich auf Zeile 5, der Aufruf der Funktion `line`.

Eine Funktion ist eine Anweisung, welche vom Computer im Hintergrund zur Verfügung gestellt. Eine Funktion ist quasi ein eigenständiges Mini-Programm, welches über dessen Namen gestartet werden kann. Über eine Liste von Werten (Komma-getrennt), können wir dem Mini-Programm genau sagen, was es für uns tun soll. In diesem Falle beschreiben die ersten beiden Werte die Koordinaten der Startpunks, und die letzten beiden Werte die Koordinaten des Endpunkts.

Mit Funktionen werden wir uns später noch vertieft auseinander setzen. Vorerst reicht es, die von PS5.js zur Verfügung gestellten Funktionen aufzurufen, um coole Dinge zu zeichen.

=== P5.js
Wenn du den Code von oben im Browser ausführst, wird das einen Fehler geben. Der Grund ist, dass es die Funktion `line` nicht gibt. Zudem gibt es keine Zeichenfläche.

Mit einem zusätzlichen Werkzeug namens P5.js erhalten wir jedoch Zugriff auf eine Zeichenfläche, sowie eine Menge von Funktionen, die wir verwenden können um darauf zu zeichnen.
...
Ich weiss, ...
====
* [*] wie ich eine Linie, ein Kreis oder ein Rechteck zeichne
====
