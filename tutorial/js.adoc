...
Hinweis: Die Funktion `keyTyped` und `Math.floor`
...
Zusatzaufgabe: Löse das Problem ohne `if`. Tip: `Math.min`.
...
Im Beispiel oben, werden die Zeilen 4 und 5 insgesamt 100 mal nacheinander ausgeführt. Begonnen wird mit `counter = 0`. Der letzte Durchgang wird mit `counter = 99` ausgeführt. Der Computer wiederholt also den Schleifeninhalt solange, bis die Abbruchbedingung `false` ist.
