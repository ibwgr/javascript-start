...
Eine weitere, wichtige Datenstruktur ist das Array. Mit einem Array können wir eine Menge von Werten, einer einzigen Variabel speichern und später auf die einzelnen Werte zugreifen.
...
Im folgenden Beispiel werden diese drei Methoden verwendet:
...
[%collapsible]
====
Generiert eine Kommaseparierte Liste von fünf zufälligen Ganzzahlen, jede davon >= 0 und \<= 10, wobei eine Zahl nie doppelt vorkommt.
====
...
Im Zusammenhang mit Arrays gibt es einen weiteren Schleifentyp, die `for of` Schleife. Diese ermöglicht es sehr einfach über alle Werte, die in einem Array gespeichert sind, zu <<iterieren>>. Man sagt auch: "über den Array iterieren" oder: "über die Elemente des Array iterieren".
...
Verwende das Codebeispiel von oben (das mit den Quizzfragen). Ändere den Code so ab, dass die Variable `i` nicht mehr gebraucht wird.
