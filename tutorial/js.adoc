...
Im Folgenden eine Tabelle, welche den Zustand des Programms bei jedem Schleifendurchgang darstellt. Also den Zustand der Variablen, jedesmal unmittelbar bevor Zeile 3 ausgeführt wird.
...
. Verändere das Programm so, dass du in einer Variable `parts` vorgeben kannst, in wieviele Spalten die Fläche unterteilt wird.
...
Verwende das Codebeispiel von oben (das mit den Quizzfragen und dem console.log). Ändere den Code so ab, dass die Variable `i` nicht mehr gebraucht wird.
...
. Generiere hundert Punkt-Objekte. Jeder Punkt besteht aus einer x- und y-Koordinate. Die x- und y-Werte sollen zufällig sein, jedoch immer ganzahlig und zwischen 10 und 990 (inklusive).
...
Im folgenden Beispiel verwenden wir eine selbst geschriebene Funktion `drawSmile`. Diese zeichnet ein Smiley. Als Parameter erwartet die Funktion, die x- und y-Koordinaten an denen das Smiley gezeichnet werden soll.
...
Die Funktion `drawSmiley` wird innerhalbt der while Schleife insgesamt 10 mal aufgerufen. Bei jedem Aufruf wird für den x- und y-Parameter jeweils eine zufällige Zahl berechnet. Nachdem die while Schleife durch ist, wird guter letzt wird noch ein Smiley in die Bildmitte gezeichnet.

Der Vorteil von Funktionen ist:

* Zusammengehörige Anweisungen können in einem sprechenden Namen zusammgenfasst werden. {nbsp} +
 Zum Beispiel: Alle Anweisungen die es braucht, um ein Smiley zu zeichnen, werden in einer Funktion namens `drawSmiley` zusammgenfasst.

* Funktionen können widerholt aufgerufen werden. Damit kann ich an verschiedenen Orten meines Programms von der Funktionalität der Funktion gebrauch machen. {nbsp} +
...
Verwende das Codebeispiel von oben (im Editor). Verändere den Code so, dass das Endergbnis gleich bleibt, jedoch ohne eine selbst geschriebene Funktion (ausser setup ;)) zu verwenden.
